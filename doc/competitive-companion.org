...
This package provides an Emacs integration to the [[https://github.com/jmerle/competitive-companion][Competitive Companion browser extension]]. It also provides some commands to speed up the coding-compiling-test loop during online contests.

The package is designed to be minimalistic and as non-intrusive as possible, while still providing some level of convenience. The idea is that during an online contest, you turn on the global minor mode. Then, when you read a problem with the browser extension, Emacs will automatically download the test cases into a hidden directory next to the implementation file, without cluttering your folder listing, and generate an implementation file using your preferred language and template in the contest's folder. When you are done implementing the code and compiling it, the package provides an interactive command that runs the program against the downloaded test cases. Then, if all outputs matched, it just lets you know in the minibuffer. Otherwise, an special buffer pops up with only the failed test cases.

* Installation

This package is currently only available through the GitHub repository. Check installation instructions [[https://github.com/luishgh/competitive-companion.el?tab=readme-ov-file#installation][there]].

* Tutorial

** Initial setup

First, download the [[https://github.com/jmerle/competitive-companion][competitive companion browser extension]] for your favorite (and supported) browser. The extension is the one responsible for reading the problem's information from the web-page.
...
Now is the perfect time to take a look at the options in [[*Customizing][Customizing]] to make sure the package behaves like you expect. If you don't want to dig into customization just yet, please take a quick look at the language/file options ([[*Language/file options][Language/file options]]), as they are the ones that must be set correctly for the package to work.

*** Evil integration

If you are an user of ~evil-mode~, you should call the function below in your configuration after loading this package:

#+findex: competitive-companion-setup-evil
- ~competitive-companion-setup-evil~ :: Set up default bindings for Evil users. This avoids that Evil supresses the package's default bindings in the outputs buffer, [[*Inside the outputs buffer][Inside the outputs buffer]].

For example, using ~use-package~ your config should look something like this:
...
3. Open a task page in your browser and click the extension button. The task's implementation file is automatically generated in the contest's folder and test cases are downloaded into a hidden directory alongside it (see the "Important observations" below for the exact naming convention).
4. Code!
...
The commands below are the main source of interaction with the package. It is strongly recommended that you use your Emacs configuration to tune them to your needs. You can create a command that creates a new contest folder and automatically turns on ~competitive-companion-mode~, another command that compiles the current file and after runs ~competitive-companion-run-tests~, etc. Check the docstrings for additional details.
...
- ~competitive-companion-run-tests~ :: prompts for a program to run against the test case of the current task. The program is run synchronously, so the feedback is showed as soon as your program finishes execution and you can abort the execution with ~C-g~ if it hangs (which probably signals an infinite loop or =TLE=). The default value for the prompt is either the last used program if that exists or is automatically generated by the lambda stored at ~competitive-companion-guess-program~. Customize this variable if the default behaviour does not suit you.
...
- Test cases are persisted on disk as a hidden sibling directory of the implementation file, named =.{filename-without-extension}= (e.g. =.a/= next to =a.cpp=), containing =input<N>.txt= / =output<N>.txt= pairs. This directory is not deleted automatically and is safe to inspect or edit directly, which is handy when debugging against a specific case outside Emacs. Re-fetching a problem overwrites the original test files but preserves any you added manually.
- The package considers only the stdout to check answers, so you don't need to worry about using stderr. Most judges do this as well, but debugging output can result in =TLE=, thus ~competitive-companion-run-tests~ will let you know if all cases succeeded and stderr is being used.
...
To edit a test case, press ~RET~ while inside its =Input= and =Expected Output= sections. This will open the underlying file on the current buffer. After you are done, just go back to the outputs buffer and revert it to rerun all test cases against the previously used program.
...
- ~competitive-companion-task-template-file~ :: Template file that is used for automatically generated task implementation files. Set to nil to disable this behaviour. See ~competitive-companion-file-generation-hook~ for total control of file generation.

#+vindex: competitive-companion-prompt-task-filename
- ~competitive-companion-prompt-task-filename~ :: If non-nil, prompt for a task filename instead of automatically generating one when a task is received from the browser extension. This option must be set *BEFORE* turning on the mode and you need to restart the mode to toggle its value. This is particularly useful if generating an automatic name for an online judge is too difficult or you are doing problems from multiple sources on the same folder.

#+vindex: competitive-companion-task-filename-generator
- ~competitive-companion-task-filename-generator~ :: Function that generates the task filename based on problem's =NAME=, i.e. the field received from the browser extension. This is used if ~competitive-companion-prompt-task-filename~ is non-nil. The default behaviour is documented in the variable's docstring and just works on most major online judges, but is not guaranteed to work on all judges as this is essentially impossible.

#+vindex: competitive-companion-insert-header
- ~competitive-companion-insert-header~ :: This boolean option controls whether the package's header is inserted in automatically generated files by ~competitive-companion-insert-default-header~. It contains basic information about the problem and shares the package's GitHub. Set to nil if this annoys you. See ~competitive-companion-file-generation-hook~ for total control of file generation.

#+vindex: competitive-companion-file-generation-hook
- ~competitive-companion-file-generation-hook~ :: Hook run when a new task file is created. The functions in this hook are run in a temporary buffer visiting the newly created file. They can use the variable ~competitive-companion-last-problem-data~ to access problem metadata, check its docstring to see more details. The default behaviour is to first insert a default header, respecting ~competitive-companion-insert-header~, and after inserting the user's template file, using the value of ~competitive-companion-task-template-file~. Note that each function should expect the point to be at the end of the buffer and leave it there after being done.

#+vindex: competitive-companion-guess-program
- ~competitive-companion-guess-program~ :: Function that guesses the program's name based on task's file name. This is used as the default value for running commands on the first time they are called. Check the docstring for behaviours the function stored is expected to have.

** Output buffer options

#+vindex: competitive-companion-collapse-test-cases
- ~competitive-companion-collapse-test-cases~ :: Controls whether test cases in the output buffer should be initially collapsed.

#+vindex: competitive-companion-separate-stderr
- ~competitive-companion-separate-stderr~ :: If non-nil, stdout and stderr are shown separately on output buffers. NOTE that stderr is never considered when validating your answer, this just affects the output display.

You can also customize the faces for each of the supported verdicts (=TLE= is included but not currently used). The faces are named ~competitive-companion-verdict-*~.
...
- ~competitive-companion-server-port~ :: Port on which the package's server listens. This must be consistent with the browser extension's configuration, [[*Initial setup][Initial setup]].

** Modeline support

This package provides a fairly simple modeline support. If enabled, the modeline will display an indicator when the mode is active. If the =nerd-icons= package is installed, the indicator is a balloon. Otherwise, it defaults to =" CC "=.

#+vindex: competitive-companion-modeline-support
- ~competitive-companion-modeline-support~ :: Enable modeline support

#+vindex: competitive-companion-modeline-face
- ~competitive-companion-modeline-face~ :: Face used by the modeline indicator
...
# Local Variables:
# before-save-hook: (org-texinfo-export-to-info)
# End:
